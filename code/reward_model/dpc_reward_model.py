...
@dataclass
class DPCRewardModelOutput(ModelOutput):
...
class DPCRewardModel(nn.Module):
    """Pairwise reward model: extracts logits at token positions for A, B, EQUAL."""
    ...
    def __init__(self, cfg, base_model, tokenizer):
        ...
        for idx, letter in enumerate(letters[: self.num_labels]):
            print(f">> DPCRewardModel: '{letter}' -> token id: {self.tok_locations[idx]}")
    ...
    def get_logits(self, input_ids, position_ids, end_idxs):
        ...
        head_logits = outputs.logits.index_select(1, end_idxs)[0]  # (num_examples, vocab_size)
    ...
    def forward(self, input_ids, position_ids, end_idxs, labels=None, **kwargs):
        ...
        if labels is not None:
            if self.label_smoothing > 0:
                n = self.num_labels
                smooth = self.label_smoothing / n
                targets = labels * (1.0 - self.label_smoothing) + smooth
            else:
                targets = labels
            loss = -torch.mean(torch.sum(torch.log_softmax(logits, dim=-1) * targets, dim=-1))

        return DPCRewardModelOutput(loss=loss, logits=logits)
